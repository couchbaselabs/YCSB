...
/**
 * Azure Cosmos DB Java SDK 4.78.0 client for YCSB.
 */

public class AzureCosmosClient extends DB implements IndexableDB {
  ...
  private void initAzureCosmosClient() throws DBException {
    ...
    debug = this.getBooleanProperty("debug", false);
  ...
  public Status update(String table, String key, Map<String, ByteIterator> values) {
    ...
    for (int attempt = 0; attempt < NUM_UPDATE_ATTEMPTS; attempt++) {
      try {
        CosmosContainer container = getContainer(table);
        CosmosItemResponse<ObjectNode> response = container.readItem(key, new PartitionKey(key), ObjectNode.class);
  ...
  public Status insert(final String table, final String key, List<DatabaseField> values_) {
    ObjectNode node = useTypedFields
      ? AzureCosmosQueryHelper.encodeWithTypes(key, values_)
      : AzureCosmosQueryHelper.encode(key, DB.fieldListAsIteratorMap(values_));
    PartitionKey pk = new PartitionKey(key);
    if(batchSize > 1) {
      // throw new UnsupportedOperationException("batch insert not yet implemented");
      return batchInsert(table, pk, node);
    }
    try {
      CosmosContainer container = getContainer(table);
      CosmosItemRequestOptions options = new CosmosItemRequestOptions();
      if (useUpsert) {
        container.upsertItem(node, pk, options);
      } else {
        container.createItem(node, pk, options);
      }
      return Status.OK;
    } catch (CosmosException e) {
      if (!AzureCosmosClient.includeExceptionStackInLog) {
  ...
  public Status aggregate(String table, String[] airports, int minOccurrences, Vector<HashMap<String, ByteIterator>> results) {
    String query = AzureCosmosQueryBuilder.buildAggregatePlaceholderQuery();
    List<SqlParameter> params = new ArrayList<>();
    AzureCosmosQueryBuilder.bindAggregatePlaceholderQuery(params, airports, minOccurrences);
    ...
    try {
      ...
      CosmosPagedIterable<ObjectNode> queryResults = container.queryItems(
        new SqlQuerySpec(query, params),
        options,
        ObjectNode.class
      );
      
      List<ObjectNode> returned = new ArrayList<>();
      Iterator<ObjectNode> it = queryResults.iterator();
      while(it.hasNext()) { 
      ...
      List<ObjectNode> best = new LinkedList<>();
      double currentMin = -1;
      for(ObjectNode obj : returned) {
        double objAvg = obj.get(AzureCosmosQueryBuilder.AVG_STOPS_FIELD).asDouble();
        if(best.size() == 0) {
          best.add(obj);
          currentMin = objAvg;
          continue;
        }
        if(best.size() >= AzureCosmosQueryBuilder.AGGREGATE_QUERY_LIMIT) {
          // there are too many elements in the list, need to remove one element if current obj is better than current min.
          // current obj is better, objAvg > currentMin
          if(objAvg > currentMin) {
            // find current min in list and remove one occurrence of it from the list.
            // list is ordered by average stops, so current min is at the end of the list.
            for(int i = best.size() - 1; i >= 0; i--) {
              if(best.get(i).get(AzureCosmosQueryBuilder.AVG_STOPS_FIELD).asDouble() <= currentMin) {
                best.remove(i);
                break;
              } else {
                // if the list is ordered by average stops, there should not be any element with average stops less than current min.
                throw new IllegalStateException("List is not ordered by average stops, found element with average stops greater than current min.");
              }
            }
          }
        }
        // we should have at least one spare slot in the list now
        if(best.size() < AzureCosmosQueryBuilder.AGGREGATE_QUERY_LIMIT) {
        // add current obj to the list in order and find new currentMin.
          for(int i = 0; i < best.size(); i++) {
            if(objAvg > best.get(i).get(AzureCosmosQueryBuilder.AVG_STOPS_FIELD).asDouble()) {
              best.add(i, obj);
              break;
            }
          }
        }
        currentMin = best.get(best.size() - 1).get(AzureCosmosQueryBuilder.AVG_STOPS_FIELD).asDouble();
      }
      if(debug) {
        System.err.println("aggregate query: " + query + ", params: " + params + ", final returned: " + best);
      }
      for(ObjectNode obj : best) {
        HashMap<String, ByteIterator> row = new HashMap<String, ByteIterator>();
        AzureCosmosQueryHelper.extractTypedFields(obj, null, row);
        results.add(row);
      }
      return Status.OK;
    } catch (CosmosException e) {
      if(includeExceptionStackInLog) {
        LOGGER.error("aggregate failed with exception:", e);
      }
      return Status.ERROR;
    }
  }
 ...
 @Override
  public Status findOne(String table, List<Comparison> filters, Set<String> fields, Map<String, ByteIterator> result) {
    ...
    String query = AzureCosmosQueryBuilder.buildFindOnePlaceholderQuery(filters);
    List<SqlParameter> params = new ArrayList<>();
    AzureCosmosQueryBuilder.bindFindOneQuery(params, filters);
    if(debug) {
      String p = "";
      for(SqlParameter param : params) {
        if(p.isEmpty()) p = "(";
        else p = p + ", ";
        p = p + param.getName() + "=" + param.getValue(Object.class).toString();
      }
      System.err.println("findOne query: " + query + ", params: " + p + ") , filters: " + filters);
    }
    try {
      CosmosContainer container = getContainer(table);
      CosmosQueryRequestOptions options = new CosmosQueryRequestOptions();
      ...
      AzureCosmosQueryHelper.extractTypedFields(results.get(0), fields, result);
      return Status.OK;
    } catch (CosmosException e) {
      if(includeExceptionStackInLog) {
        LOGGER.error("findOne failed with exception:", e);
      }
      return Status.ERROR;
    }
  }
  ...
  public Status updateOne(String table, List<Comparison> filters, List<DatabaseField> fields) {
    ...
    String query = AzureCosmosQueryBuilder.buildUpdateOnePlaceholderQuery(filters, fields);
    List<SqlParameter> params = new ArrayList<>();
    AzureCosmosQueryBuilder.bindUpdateOneQuery(params, fields, filters);
    if(debug) {
      System.err.println("updateOne query: " + query + ", params: " + params);
    }
    try {
      CosmosContainer container = getContainer(table);
      CosmosQueryRequestOptions options = new CosmosQueryRequestOptions();
      ...
      // get the id and partition key of the record to update
      ObjectNode node = results.get(0);
      String id = node.get("id").asText();
      PartitionKey pk = new PartitionKey(id);
      
      
      for(DatabaseField field : fields) {
        AzureCosmosQueryHelper.fillDocument(field, node);
      }
      node.remove("_rid");
      node.remove("_self");
      node.remove("_etag");
      node.remove("_attachments");
      node.remove("_ts");
      if(debug) {
        System.err.println("updateOne for node: " + pk + ", updated document: " + node);
      }
      container.replaceItem(node, id, pk, new CosmosItemRequestOptions());
      
      return Status.OK;
    } catch (CosmosException e) {
      if(includeExceptionStackInLog) {
        LOGGER.error("updateOne failed with exception:", e);
      }
      return Status.ERROR;
    }
  }

  private CosmosContainer getContainer(String table) {
    if (!containerCache.containsKey(table)) {
      CosmosContainer container = database.getContainer(table);
      containerCache.put(table, container);
    }
    return containerCache.get(table);
  }

  private final List<CosmosItemOperation> bulkInserts = new ArrayList<>();
  private Status batchInsert(final String table, final PartitionKey pk, ObjectNode node) {
    // bulkInserts.put(key, encoding);
    CosmosItemOperation op = useUpsert
      ? CosmosBulkOperations.getUpsertItemOperation(node, pk)
      : CosmosBulkOperations.getCreateItemOperation(node, pk);
    bulkInserts.add(op);
    if(bulkInserts.size() < batchSize) {
      return Status.BATCHED_OK;
    }
    // At this point, bulkInserts contains batchSize number of records to insert.
    // We will attempt to insert them in a batch, and if any of them fail, we will
    // log the failure and return an error status. If all of them succeed, we will return OK.
    try {
      CosmosContainer container = getContainer(table);
      Iterable<CosmosBulkOperationResponse<Object>> responses = container.executeBulkOperations(bulkInserts);
      bulkInserts.clear();
      for(CosmosBulkOperationResponse<Object> response : responses) {
        if(!response.getResponse().isSuccessStatusCode()) {
          ...
          LOGGER.error("batch insert failed for operation: " + response.getOperation() + ", error: " + message + ", status code: " + response.getResponse().getStatusCode());
        }
      }
      return Status.OK; 
    } catch (CosmosException e) {
      if(includeExceptionStackInLog) {
        LOGGER.error("batch insert failed with exception:", e);
      }
      return Status.ERROR;
    }
    ...
    try {
      ...
      for(Map.Entry<String, Map<String,Object>> entry : localInserts.entrySet()) {
        try {
          if (useUpsert) {
            container.upsertItem(entry.getValue(), new PartitionKey(entry.getKey()), options);
          } else {
            container.createItem(entry.getValue(), new PartitionKey(entry.getKey()), options);
          }
        } catch (CosmosException e) {
          if(includeExceptionStackInLog) {
            LOGGER.error("one insert failed in batch insert loop: " + entry.getKey(), e);
          }
          errorCount++;
        }
      }
